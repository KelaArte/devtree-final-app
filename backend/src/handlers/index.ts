...
export const uploadImage = async (req: Request, res: Response) => {
    ...
    form.parse(req, (error, fields, files) => {
        //console.log(files) //para que no tenga que acceder al objeto
        cloudinary.uploader.upload(files.file[0].filepath, { public_id: uuid() }, async function (error, result) {
            //va a ser async porque va a interactuar con la api o cloudinary
            //console.log(error)
            //console.log(result)
            if (error) {
                const error = new Error("Hubo un erroral subir la imagen")
                return res.status(500).json({ error: error.message })
            }
            if (result) {
...
export const checkHandleAvailability = async (req: Request, res: Response) => {
  try {
    ...
    console.error(e)
    return res.status(500).json({ error: e.message || 'Error comprobando handle' })
  }
}